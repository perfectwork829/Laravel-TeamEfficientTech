    ...
    public function index()
    {
        ...
        foreach ($projects as $project) {
            if ($project->project_id && $project->project) {
                $begin = Carbon::createFromFormat(config('panel.date_format') . ' ' . config('panel.time_format'),
                    $project->start_time);
                $end = Carbon::createFromFormat(config('panel.date_format') . ' ' . config('panel.time_format'),
                    $project->end_time);
