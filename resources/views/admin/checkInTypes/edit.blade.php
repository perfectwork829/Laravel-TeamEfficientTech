        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.checkInType.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.check-in-types.update", [$checkInType->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('check_in_type') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="check_in_type" id="check_in_type" value="{{ old('check_in_type', $checkInType->check_in_type) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('notes') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="notes" id="notes">{{ old('notes', $checkInType->notes) }}</textarea>
                            @if($errors->has('notes'))
