                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.trackings.update", [$tracking->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('lat') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="lat" id="lat" value="{{ old('lat', $tracking->lat) }}" required>
                            @if($errors->has('lat'))
                        ...
                        <div class="form-group {{ $errors->has('lon') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="lon" id="lon" value="{{ old('lon', $tracking->lon) }}" required>
                            @if($errors->has('lon'))
                        ...
                        <div class="form-group {{ $errors->has('action') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="action" id="action">
                                ...
                                @foreach(App\Models\Tracking::ACTION_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('action', $tracking->action) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
